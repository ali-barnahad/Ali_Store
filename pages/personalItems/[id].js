...
const Product = React.memo(({ product, comments, mostVisitedProducts }) => (
  <>
    <Head>
      ...
      <meta
        property="og:url"
        content={`https://alistore.liara.run/personalItems/${product._id}`}
      />
      <meta property="og:image" content={product.imageURL || defaultImg} />
      <link
        rel="canonical"
        href={`https://alistore.liara.run/personalItems/${product._id}`}
      />
        ...
        {`
          {
            ...
            "url": "https://alistore.liara.run/personalItems/${product._id}",
            "offers": {
              "@type": "Offer",
              "priceCurrency": "USD",
              "price": "${product.price}",
    ...
    <BreadcrumbsComponent route={`/personalItems/${product.title}`} />
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    const mostVisitedProducts = await personalItemModel
      .find({})
      .sort({ view: -1 })
      .limit(10)
      .lean();
