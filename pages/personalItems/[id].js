...
const Product = React.memo(({ product, comments }) => (
  <>
    <Head>
      <title>{`${product.name} - Buy Online at Ali Store`}</title>
      <meta
        name="description"
        content={`Purchase ${product.name} at Ali Store. ${product.description}`}
      />
      <meta name="robots" content="index, follow" />
      <meta
        property="og:title"
        content={`${product.name} - Buy Online at Ali Store`}
      />
      <meta
      ...
      <meta
        property="og:url"
        content={`https://sticker-next.liara.run/personalItems/${product._id}`}
      />
      <meta
        property="og:image"
        content={
      ...
      <link
        rel="canonical"
        href={`https://sticker-next.liara.run/personalItems/${product._id}`}
      />
...
export async function getStaticPaths() {
  ...
  try {
    const products = await personalItemModel.find({});
...
export async function getStaticProps({ params }) {
  await connectToDB();
  try {
    const product = await personalItemModel.findById(params.id).lean();
    if (!product) {
