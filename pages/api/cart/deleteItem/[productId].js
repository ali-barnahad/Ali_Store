...
import ShoppingCartItem from "@/models/shoppingCartModel";
...
export default async function handler(req, res) {
  ...
  let decoded;
  try {
    decoded = await verifyAccessToken(token); // Adjust this line if  is not async
  } catch (error) {
  ...
  try {
    ...
    const deletedItem = await ShoppingCartItem.findOneAndDelete({
