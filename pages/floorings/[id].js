...
const Product = React.memo(({ product, comments, mostVisitedProducts }) => (
  <>
    <Head>
      ...
      <meta
        property="og:url"
        content={`https://alistore.liara.run/floorings/${product._id}`}
      />
      <meta property="og:image" content={product.imageURL || defaultImg} />
      <link
        rel="canonical"
        href={`https://alistore.liara.run/floorings/${product._id}`}
      />
    ...
    <BreadcrumbsComponent route={`/floorings/${product.title}`} />
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    const mostVisitedProducts = await flooringModel
      .find({})
      .sort({ view: -1 })
      .limit(10)
      .lean();
